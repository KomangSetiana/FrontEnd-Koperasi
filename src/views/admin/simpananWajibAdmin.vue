<template>
    ...
    <div class="flex">
        ...
        <div class="p-8 h-screen overflow-y-auto flex-1">
            ...
            <div class="overflow-y-auto rounded-lg shadow hidden md:block">
                <table class="w-full" id="download-document">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="(item, index) in data" :key="index" class="hover:bg-slate-300"
                            ...
                            <td class="p-2 text-sm text-gray-700 whitespace-nowrap" :class="[item.saldo % 120000 == 0
                                ? 'text-green-500' : 'text-red-500']">{{ item.saldo % 120000 == 0 ? 'lunas' :
                                    'belum lunas' }}</td>
                            <td>
                                <router-link :to="{ name: 'show simpanan wajib', params: { id: item.member_id } }">
