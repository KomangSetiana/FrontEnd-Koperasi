                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="(item, index) in report" :key="index" class="hover:bg-slate-300"
                            ...
                            <td class="p-2 text-sm text-gray-700 whitespace-nowrap">
                                <router-link :to="{ name: 'laporan-simpanans', params: { id: item.id } }"
                                    :class="[item.type == 'angsuran' && 'hidden']"
                                    class="ml-3 text-white bg-blue-600 hover:bg-blue-400 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">
                                    Details
                                </router-link>
                                <router-link :to="{ name: 'laporan-angsurans', params: { id: item.id } }"
                                    :class="[item.type == 'deposit' && 'hidden']"
                                    class="ml-3 text-white bg-blue-600 hover:bg-blue-400 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">
                                    Details
                                </router-link>
                                <button @click="validasiLaporan(item.id)"
                                    :class="[item.is_validasi == false ? 'bg-red-500 hover:bg-red-400' : 'bg-green-500 hover:bg-green-400']"
                                    class="ml-3 text-white font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">
                                    Validasi
                                </button>
                            </td>
                        </tr>
...
const getReport = () => {
    ...
    axios.get("http://127.0.0.1:8000/api/leader-report", {
        headers: {
...
const validasiLaporan = async (id) => {
    axios.put("http://127.0.0.1:8000/api/validasi/" + id, {
        headers: {
            Authorization: 'Bearer' + localStorage.getItem('token')
        }
    })
